...
└──────────────────────┬──────────────────────────────────┘
                       ...
                       ▼
┌──────────────────────────────────────────────────────────┐
│          Vercel Edge Network (Next.js Hosting)           │
│  ┌────────────┐  ┌────────────┐  ┌────────────┐        │
...
# Chapa
CHAPA_SECRET_KEY=CHASECK-xxxxx # SECRET!
CHAPA_WEBHOOK_SECRET=xxx # SECRET!
...
Monitor 3: Payment Webhook
  URL: https://fabrica.et/api/webhooks/chapa/health
  Type: HTTPS
  Interval: 5 minutes
  Alert: Email + Slack
```
...
```bash
# Supabase Dashboard → Database → Backups → Enable PITR
```
...
1. Check Chapa status: https://status.chapa.co (if available)
2. Verify webhook endpoint accessible: `curl https://fabrica.et/api/webhooks/chapa/health`
...
3. Review recent deployments → **Rollback immediately**
